...
- `v2algodclient_responses.feature` and `v2indexerclient_responses.feature` cucumber unit tests are now live. They serve canned HTTP response bodies (the upstream fixtures under `tests/features/resources/`) from a new in-process `ResponseMockServer` (`tests/step_defs/unit/mock_server.rs`), drive a high-level `Algod` / `Indexer` call, and assert on the parsed response — no Algorand harness required. The new step-def module is `tests/step_defs/unit/v2_client_responses.rs`. Documented exclusions only: the msgpack-bodied algod scenarios (the generated `get_block` / pending-transaction endpoints parse responses as JSON only), the algod `Get Block` heartbeat scenarios (the `algonaut_algod` `BlockResponse` transaction enum has no `hb` variant), and the indexer scenarios whose fixtures predate fields the current generated `algonaut_indexer` models mark mandatory (`metadata-hash`, `total-apps-opted-in`, `transactions-root-sha256`)
