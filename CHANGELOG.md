...
- The `contract!` macro: `algonaut::contract!("path/to/contract.json")` reads an ARC-4 ABI JSON file at compile time and generates a typed client — a struct named after the contract (PascalCase), one `snake_case` builder method per ABI method, and network constructors (`new(app_id, sender, signer)` plus `testnet`/`mainnet`/`betanet`). Scalar ABI argument types (`uintN`, `bool`, `address`, `string`, `byte[]`) map to their Rust counterparts, and an unsupported type yields a `compile_error!` with guidance. Two new crates back it: the `algonaut_abi_macros` proc-macro crate (re-exported as `algonaut::contract!`) and `algonaut_abi_model`, a dependency-light leaf crate holding the canonical serde model for the app-spec JSON so the runtime and the macro cannot disagree on the wire format. The ARC-56 app-spec support, typed-struct arguments, `deploy`, state readers, and ARC-28 events documented below all extend this ARC-4 base. See `docs/adr/contract-macro-from-abi-json.md` (#341)
- The `contract!` macro now generates state read accessors for local state, boxes, and maps, beyond the existing `global_<key>` getters: `local_<key>(&Algod, &Address)` reads a declared local-state key for an account; `box_<key>(&Algod)` reads a declared box through algod's box endpoint (an absent box reads as `None`); and `<class>_<map>(&Algod, [&Address,] key)` reads a global/local/box map entry, encoding the runtime `key` per the map's declared `keyType` (AVM types raw, ABI types ABI-encoded) and prepending the map prefix. All accessors are `async`, return the value decoded per its declared ARC-56 type as `Option<AbiValue>`, and skip any location whose key/value type the macro can't decode. Box values are read as a flat byte buffer (no TEAL type tag), so AVM `uint64` decodes from its big-endian bytes (part of #345)
- The `contract!` macro now auto-supplies sourced (non-literal) `defaultValue` arguments by reading them at call time, extending the existing `literal`-default support. A method with a `global`/`box`/`local`/`method`-sourced default becomes `async`, takes an `&Algod`, and returns its builder once the defaults resolve: `global`/`box` read the declared storage, `local` reads the client's configured sender, and `method` simulates the named read-only method and takes its return value. As with literal defaults, the defaulted argument is dropped from the method's parameter list. Methods with only literal defaults (or none) keep their synchronous signature (part of #345)
- The `contract!` macro now generates a typed client from a full ARC-56 "Extended App Description" — `contract!("path/to/app.arc56.json")` — extending the previous ARC-4-only support (an ARC-4 contract JSON still works unchanged). Beyond the per-method builders and network constructors it already emitted, from one app-spec file it now also generates: named ARC-56 `structs` as typed Rust arguments, including inline-nested structs emitted recursively as `<Parent><Field>`; a `deploy` constructor that compiles the spec's TEAL `source`, substitutes integer `TMPL_` template variables (one typed `deploy` parameter each), and creates the app through its ABI `createApplication` method or a bare create, returning a client bound to the new app id; `global_<key>` global-state read accessors decoded per each key's declared ARC-56 type; a read-only `simulate` path on every method builder (the ARC-22 read path — it signs with placeholder signatures and submits nothing); a `<Contract>Event` enum with a `decode_events` decoder for the spec's ARC-28 `events`; lifecycle setters (`.opt_in()`, `.close_out()`, `.update()`, `.delete()`) gated by each method's declared `call` actions; and automatic literal default arguments — an argument with a `literal` `defaultValue` is supplied for the caller, so the generated method drops that parameter. Methods whose argument types the macro cannot model yet (transaction, reference, or unsupported-struct arguments) are omitted from the client and listed in its doc comment, so a real-world spec yields a usable partial client instead of a `compile_error!`. See `docs/adr/contract-macro-arc56-app-spec.md`
- `algonaut_abi_model` now parses the full ARC-56 app spec — named `structs`, declared `state` (global/local/box schema and keys), ARC-28 `events`, per-method `actions`/`readonly`, per-argument `struct`/`defaultValue`, and deploy metadata (`source`/`byteCode`/`compilerInfo`, template and scratch variables) — alongside the ARC-4 `name`/`methods`/`networks`. Every ARC-56-only field is optional with a serde default, so an ARC-4 document parses unchanged and re-serializes byte-identically. The model is validated against the upstream ARC-56 JSON schema and round-tripped against real AlgoKit specs (Reti, NFD, ZeroCouponBond, and the ARC-56 reference fixtures) in the crate's tests
- `ExecuteOutcome::created_app_id: Option<AppId>` — the id of the application created by an executed atomic group's create transaction (e.g. an app-create / deploy), or `None` when the group created none. The generated `deploy` constructor reads it to bind its client
- A runnable end-to-end example, `examples/contract_arc56.rs`, that drives a generated `Vault` client through deploy, an atomic group of typed-struct calls, a read-only simulate, typed global-state reads, and ARC-28 event decoding against a local node — backed by a real Vault ARC-56 fixture and its TEAL. A `make sandbox` target boots a fast prebuilt-image algod+kmd to run it (and the new ARC-56 `e2e` test suite) against
- The `contract!` macro now generates methods with ARC-4 reference arguments — `account` → an `Address` parameter, `asset` → an `AssetId`, `application` → an `AppId`. The value flows through `MethodInvocation` as a plain `AbiValue`; at group-build time the method-call encoder appends it to the application call's foreign accounts/assets/apps array and encodes the `uint8` index as the ABI argument. Such methods were previously omitted from the generated client (closes #346)
- The `contract!` macro now generates methods with ABI array arguments — dynamic `T[]` maps to `Vec<R>` and static `T[N]` to `[R; N]` (with `R` the Rust mapping of the element type `T`), each element encoded recursively and wrapped in `AbiValue::Array` so nested arrays (e.g. `uint64[][]`) work too. `byte[]` keeps its canonical `Vec<u8>` mapping, and array-typed named-struct fields are likewise supported. Methods previously omitted solely for an array argument are now generated (closes #347)
- The `contract!` macro now generates methods with anonymous ABI tuple arguments and `ufixedNxM` arguments, closing the last value-type gaps from #342. An anonymous tuple `(T1,T2,…)` maps to a Rust tuple `(R1,R2,…)` (each element mapped recursively), encoded into the `AbiValue::Array` representation named ARC-56 structs already use (a struct is a tuple under the hood). A `ufixedNxM` maps to a new `Ufixed<N, M>` value newtype (in `algonaut_abi::macro_support`) that carries its `M`-decimal-place scale in the type and wraps the raw, unscaled `N`-bit integer (`round(real * 10^M)`); it encodes to `AbiValue::Int` exactly as a `uintN` of the same width does — no new dependency or `AbiValue` variant. Tuple- and `ufixed`-typed named-struct fields are likewise supported. See `docs/adr/contract-macro-ufixed-args.md`
- Generated method-call builders now expose `.box_ref(key)` and `.box_ref_of(app, key)` to attach box references to a call, so stateful methods that read or write boxes can be invoked; the reference flows to the application-call transaction. Dynamic `boxMap` key derivation remains future work — the caller supplies the raw box key bytes (closes #348)
- The `contract!` macro now generates methods with ARC-4 transaction arguments (`pay`/`axfer`/`txn`/…): such a method takes a caller-supplied `TransactionWithSigner` that the builder places immediately before the method call in the atomic group. Generated builders now carry an `atomic::Invocation` (a superset of the previous `MethodInvocation`) to hold the transaction-typed argument (closes #349)
- Typed value decoding for the `contract!` macro: each generated ARC-56 named struct now gains an `abi_decode(AbiValue) -> Result<Self, AbiDecodeError>` that is the exact inverse of its `abi_encode` (recursing into named/nested struct fields, range-checking integers, and erroring on a shape or arity mismatch). Every method builder additionally gains an async `simulate_decoded(algod, params)` that dry-runs the call and returns the ABI return value already decoded into its typed Rust representation — the generated struct when the return carries a `struct` overlay, a scalar/array otherwise — beside the existing raw `simulate`. Backed by a new `AbiDecode<Marker>` trait and `AbiDecodeError` in `algonaut_abi::macro_support`, the symmetric dual of the `AbiArg<Marker>` encode path. See `docs/adr/contract-macro-typed-decoding.md` (part of #345)
- `ExecuteOutcome::created_app_ids: Vec<(usize, AppId)>` and `ExecuteOutcome::created_app_id_at(index)` — every application an executed atomic group created, paired with the group index of the creating transaction, so a create at any slot of a mixed group is observable (not only the awaited transaction that `created_app_id` reflects) (part of #345)
- The generated `deploy` constructor now supports specs that ship only precompiled `byteCode` (no TEAL `source`): it builds the approval/clear programs directly from the base64 byteCode instead of compiling through algod. (A `byteCode`-only spec that also declares TEAL template variables still gets no `deploy`, since templates substitute into source text a compiled program no longer carries.) `deploy` is also richer: when the spec declares an ABI create method, its typed constructor arguments become typed `deploy` parameters (encoded by the same machinery as regular method arguments — scalars, named structs, arrays, references, and literal defaults all behave identically), the declared create OnComplete is honored (not just `NoOp`), reference-typed create arguments populate the create transaction's foreign arrays, and the number of extra program pages is sized automatically from the compiled program length so large contracts deploy without a new parameter. The `deploy` signature stays additive: existing four-argument and template-variable call sites are unchanged; create-method parameters only appear when the spec declares them. Create-time box references, arbitrary extra foreign references, and a fluent deploy builder remain future work. See `docs/adr/contract-macro-arc56-deploy.md` (part of #345)
