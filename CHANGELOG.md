...
- `v2algodclient_paths.feature` (36 scenarios) and `v2indexerclient_paths.feature` (57 scenarios) cucumber unit tests are now live. They exercise the request paths the algod / indexer v2 clients emit against a new in-process recording mock HTTP server (`tests/step_defs/unit/mock_server.rs`), with no Algorand harness required. The `features_runner` matrix gains an `excluded_scenarios` field for name-qualified per-scenario exclusion; a handful of scenarios are excluded for documented capability gaps (a missing `header-only` / `online-only` / `group-id` query parameter) or because the SDK emits query parameters in declaration order rather than the alphabetical order the upstream fixture's literal paths assume
