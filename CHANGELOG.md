...
- **Breaking:** the `algonaut::openapi_algod`, `algonaut::openapi_indexer`, and `algonaut::openapi_kmd` re-exports are removed. Reach models through `algonaut::model::{algod,indexer}` and the raw operations through `algonaut_algod::apis` / `algonaut_indexer::apis` (the client crates remain dependencies). No `openapi_*` path appears in user code or rustdoc anymore
