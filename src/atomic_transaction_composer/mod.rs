...
impl AtomicTransactionComposer {
    ...
    pub fn add_method_call(&mut self, call: MethodCall) -> Result<(), Error> {
        ...
        let mut tx_builder = TxnBuilder::with_fee(&call.suggested_params, call.fee, app_call);
        if let Some(rekey_to) = call.rekey_to {
            tx_builder = tx_builder.rekey_to(rekey_to);
        }
        if let Some(lease) = call.lease {
            tx_builder = tx_builder.lease(lease);
        }
        if let Some(note) = call.note.clone() {
