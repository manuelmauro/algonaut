...
**Labels:** `area/algod`, `area/openapi`, `kind/feature`, `cucumber-blocker`
**ADR:** [`simulaterequest-model-needs-power-pack-fields`](simulaterequest-model-needs-power-pack-fields.md)

```markdown
The autogenerated `algonaut_algod::models::SimulateRequest` only carries
`txn_groups`. The algod OpenAPI spec at HEAD additionally defines:
...
- `round: u64`, `fix-signers: bool` (newer revisions)

The response models (`SimulateTransactionGroupResult`,
`SimulateTransactionResult`) are similarly missing `exec-trace` and the
per-unit trace payload.

`tests/features/integration/simulate.feature` cannot run without these
fields — the missing pieces are listed in ADR
`simulaterequest-model-needs-power-pack-fields`.

### Acceptance
- [ ] Regenerate algod models against a pinned spec commit OR extend
      the affected files by hand.
- [ ] Add a `SimulateRequestBuilder` newtype with fluent setters.
- [ ] Add the exec-trace types (`SimulateTraceConfig`,
      `SimulationTransactionExecTrace`, `SimulationOpcodeTraceUnit`,
      `ApplicationStateOperation`).
- [ ] `simulate.feature` scenarios run green in `cargo test --test
      features_runner -- --` after the ADR is marked Accepted.
```
